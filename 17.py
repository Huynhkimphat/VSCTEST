# datetime Module
import datetime
import pytz
today = datetime.date.today()
print(today)

birthday = datetime.date(2001, 8, 27)
print(birthday)


days_since_birth = (today - birthday).days
print(days_since_birth)


tdelta = datetime.timedelta(days=10)
print(today+tdelta)

print(today.month)
print(today.weekday())  # 0-1-2-3-4-5-6 Monday=0 Sunday=6


print(datetime.time(7, 2, 20, 15))
# datetime.date(Y,M,D)
# datetime.time(h,m,s,ms)
# datetime.datetime(Y,M,D,h,m,s,ms)

# 10 hours added to current day
hour_delta = datetime.timedelta(hours=10)
print(datetime.datetime.now()+hour_delta)

# cast time to Pacific time zone
datetime_today = datetime.datetime.now(tz=pytz.UTC)
datetime_pacific = datetime_today.astimezone((pytz.timezone('US/Pacific')))
print(datetime_pacific)

# print all time zones
# for tz in pytz.all_timezones:
#     print(tz)


# string format with date
# 2020-04-29-> April 29,2020
# strftime(f=formating)

print(datetime_pacific.strftime('%B %d,%Y'))

# April 29,2020-> datetime(2020,4,29)
# strptime(p=parsing)
datetime_thing = datetime.datetime.strptime('April 29,2020', '%B %d,%Y')
print(repr(datetime_thing))
